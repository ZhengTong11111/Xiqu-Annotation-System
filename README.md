...
这是一个面向戏曲研究、表演分析、唱词校勘和多轨时间标注的昆曲多模态数据平台。它由桌面式资源管理器、账号与逐文件权限后端，以及现有精细时间轴编辑器组成。它不是普通字幕编辑器，而是把视频、句级念白/唱与角色行当、逐字时间与四声、自定义腔格/动作轨、附属打点、音频波形、人声频谱图和工尺谱附属轨放在同一套研究工作流中管理和编辑的专业工具。
...
按 30 秒生成多层波形、两档频谱和 voiced-only F0；前端只请求当前时间窗附近的瓦片。分析任务失败不会阻断
视频播放。免登录/本地编辑器仍保留浏览器分析，但单个本机媒体限制为 256 MiB，超限时会明确提示改用平台
后台分析或较小音频。
...
导入视频后，工具会尝试提取音频并显示波形。平台文件使用后台分析资产，本地文件使用有上限的浏览器分析。
波形可用于：
