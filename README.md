...
下次保存会重新取锁。整轨创建/删除、附属点轨生命周期、会同步改块类型的类型选项编辑和批量导入仍按后续
切片迁移，普通无租约编辑行为不变。
...
原子 revision 事实。自定义轨道元数据、递归分叉和块分叉归属使用独立结构命令；其 undo 记录 inverse，
redo 复用正向命令并同样先取得租约。四声/唱腔修改、分句合句、批量导入和其余未迁移结构编辑仍记录
legacy 摘要。
...
已有 pending operations、幂等 operation log、version 1 时间、稳定内容、实体生命周期、复合状态与依赖事务命令、revision 冲突、同步状态和
