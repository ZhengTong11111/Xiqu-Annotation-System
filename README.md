...
事实；整轨删除会同时原子处理工尺删除和板眼断链。轨道排序、内建轨配置、附属点轨类型联动和批量导入仍
按后续切片迁移，普通无租约编辑行为不变。
...
类型选项与块类型联动使用有界结构事务；它们的 undo 记录 inverse，redo 复用正向命令并同样先取得租约。
四声/唱腔修改、分句合句、内建轨结构、附属点轨类型联动、批量导入和其余未迁移结构编辑仍记录
legacy 摘要。
