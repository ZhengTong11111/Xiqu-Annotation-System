...
VOD ID、区域、媒体种类和时长等稳定信息，不保存 AccessKey、Secret、playauth 或临时播放地址。
...
上传媒体和阿里云 VOD 是三条长期并存的流程。当前版本已完成 VOD 资源创建、绑定和短时播放会话 API；
编辑器内的 Aliplayer 适配将在 R3h3 启用，因此已绑定 VOD 目前会明确显示“媒体暂不可播放”，不会误弹
本地视频重关联提示。
