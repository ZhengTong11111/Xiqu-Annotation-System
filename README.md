...
权限，只读文件继续以只读模式进入。片段合并仍在后续独立阶段。

当前代码已具备片段整合的只读预检核心：它能识别所选实体必须携带的句级、父文字块、轨道、工尺谱、
板眼区段和附属点定义，并报告新增、目标冲突、目标已相同或坏引用。该能力尚未开放为界面命令，也
不会自动修改任一标注文件；正式选择、冲突确认和一次性可撤销写入会在后续独立阶段完成。
