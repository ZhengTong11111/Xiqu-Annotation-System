...
不会直接覆盖服务器。正常保存成功后自动清除恢复草稿。该机制仍不等于服务器自动保存；后台保存调度、
联网重试退避和实时多人同步尚未实现。
...
必须经过编辑器确认和普通 revision save。WebSocket、presence、实时 operation 合并、自动保存调度和
联网重试尚未实现。多人同时保存同一 revision 时，后到者会收到冲突提示。
