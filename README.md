...
当前版本同时支持“不登录的本地 JSON 编辑”和“PostgreSQL + Fastify 平台资源管理”两种模式。平台模式已经支持项目/文件夹/标注文件/媒体文件、服务器保存、恢复快照、逐资源账号权限，以及认证后的单文件 revision 实时通知；多人 presence、远端选区、实时编辑命令合并、批量授权和完整服务器部署能力仍在继续建设。工尺谱传统字形渲染可用于研究预览，但正式发布前仍需要处理字体授权和替换问题。
...
也不成为第二条保存通道。当前通知 hub 仍是单 API 进程内存实现，跨实例分发、presence、远端光标和实时
dirty operation 合并尚未实现；多人同时保存同一 revision 时，后到者不会覆盖前者。
