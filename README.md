...
当前版本同时支持“不登录的本地 JSON 编辑”和“PostgreSQL + Fastify 平台资源管理”两种模式。平台模式已经支持项目/文件夹/标注文件/媒体文件、服务器保存、恢复快照、逐资源账号权限，以及认证后可跨 API 实例分发的单文件 revision 实时通知、在线成员列表和远端播放头预览；远端鼠标/选区摘要、实时编辑命令合并、批量授权和完整服务器部署能力仍在继续建设。工尺谱传统字形渲染可用于研究预览，但正式发布前仍需要处理字体授权和替换问题。
...
同一标注文件的已认证会话现在会以独立瞬时通道广播远端播放头时间和播放/暂停状态：浏览器最多每秒发送
8 次变化，静止时每 2 秒保活；服务端再次限流，并通过第三条 schema 隔离的 PostgreSQL LISTEN/NOTIFY
通道跨实例转发。慢消费者会丢弃瞬时帧，断线会发送 clear，浏览器超过 6 秒未收到更新也会回收旧播放头。
同账号多窗口按最近活动窗口合并，Timeline 只渲染其他账号、最多 32 条只读纵线。纵线直接使用现有
`trackHeaderWidth + time * zoom` 时间坐标，不参与吸附、选择、拖动或保存。该通道不传标注正文，也不进入
ProjectData、revision、恢复草稿、快照、operation log 或治理审计。远端鼠标/选区摘要和实时 dirty
operation 合并尚未实现；多人同时保存同一 revision 时，后到者不会覆盖前者。
