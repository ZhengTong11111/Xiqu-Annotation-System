...
- **P5b 本地负载与多账号浏览器验收**：在隔离数据库/对象根运行快速滚动、重复请求/取消、worker kill/restart、
  多账号任务中心和协作核心闭环；VOD 浏览器检查只使用 `http://localhost:5173/`。
...
| P5 | 进行中（P5b） | P5a 已完成候选 release/隔离 migration 门禁；下一步做本地负载与多账号浏览器验收，生产部署仍需用户明确授权 |
