...
- **P5b 本地负载与多账号浏览器验收（已完成）**：在隔离数据库/对象根验证快速滚动、24 路共享请求、12 路取消/
  重试幂等重放和 8 轮 worker 启停；管理员任务中心、系统诊断与 localhost VOD 播放/随机跳转通过。第二个普通账号
  浏览器会话当时未登录，因此没有伪造双账号 UI 结论；权限、共享执行与协作核心由隔离数据库集成测试覆盖。
...
- **P5d 生产切换与人工验收**：仅在用户明确授权后连接生产，以 `platform.admin` 维护、停止 worker、验证一致备份、
  ...
  waveform/spectrogram/F0、有效窗口渐进加载、音轨范围外留空和缓存回访；第二隔离会话已到登录门禁，仍需取得敏感
  凭据输入授权后完成双账号 UI。P5d 不得由滚动 goal 默认为已授权。
...
| P5 | 待用户授权（P5d） | P5a-P5c 已完成候选门禁、负载验收、隔离全链路彩排、部署维护窗口复用和原子切换/回滚；最新 `ff45b1b` 候选复核通过，localhost VOD 分析 UI 已补验，双账号 UI 仍等待隔离会话登录授权；P5d 生产连接和切换必须由用户明确授权 |
