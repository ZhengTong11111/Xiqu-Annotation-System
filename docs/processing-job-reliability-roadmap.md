...
- **P4a-P4d 已完成**：P3 已由 commit `a122acf` 独立提交。P4 依据现有维护门禁、对象流、存储补偿和 worker 测试基础拆成
...
- P4d 新增单一任务可靠性聚合边界，以活动任务和最近 60 分钟终态计算最老排队/心跳/取消年龄、陈旧 claim、近期
  结果和平均时长；worker 恢复、系统诊断、Prometheus 和告警共享 2 分钟陈旧阈值。历史 failed 总数继续保留，但
  只由最近失败触发当前告警。管理员诊断新增聚合结论，具体取消/重试仍由既有任务中心负责；空队列 worker 存活
  由 systemd 判断，不伪造 API 在线指标。新增 `docs/processing-job-operations.md` 区分读取、写许可、数据库、对象存储
  与 worker 故障。P4 聚合故障矩阵、完整 API 239/239、部署 28/28、构建和 localhost/Chrome 视觉验收通过；未部署生产。
...
- **P5a 候选 release 与隔离迁移门禁（已完成）**：增加只读候选目录检查，验证运行文件、workspace 产物、Prisma schema/
  migrations 和 node_modules 完整，拒绝 `.env`、`data/`、备份、草稿及指向 release 外部的 workspace 符号链接；复用
  `release:check`、正式 migration 和既有 backup/restore drill，不建立第二套发布或恢复模型。
- **P5b 本地负载与多账号浏览器验收（已完成）**：在隔离数据库/对象根验证快速滚动、24 路共享请求、12 路取消/
  重试幂等重放和 8 轮 worker 启停；管理员任务中心、系统诊断与 localhost VOD 播放/随机跳转通过。后续补验使用
  Chrome 管理员和 in-app browser 教师两个真正隔离会话打开同一验收副本，双方显示 2 人在线、可编辑、已同步；教师
  UI 只显示“我的任务/相关任务”，没有账号、项目权限、审计或系统诊断入口。跨账号正式原子命令把文件从 v17 推进到
  v18 并由教师 UI 无刷新追赶，再由权威 `teacher` 账号写回 v19、恢复原值。因 Chrome 扩展中途断开且合并块自动化
  双击不稳定，本轮不把“两端都由鼠标直接编辑”伪装成已通过；浏览器在线/追赶证据与集成测试共同覆盖协作核心。
- **P5c 生产候选演练包（已完成）**：修正现有手册中维护排空晚于一致备份、示例遗漏 worker/备份/候选检查的危险顺序；生成
  脱敏上线记录与回滚检查单，并以期望旧 release 为并发前提提供受校验的原子 symlink 切换/回滚工具。使用临时 release
  树和唯一隔离 schema/对象目录演练候选检查、维护排空、worker 停止、同操作员维护窗口内备份与离线校验、migration、
  切换/回滚、只读 smoke、显式解除维护和 worker 恢复，不连接生产。彩排据实修复了备份 CLI 不能复用部署维护窗口的
  合同矛盾：独立备份仍自行管理窗口，部署备份必须显式使用 `--require-existing-maintenance` 且不得解除外部窗口。最终
  本地审计又从 `13f1cda` 运行树组装唯一候选，候选内部确认 27 个运行路径、25 个生产依赖、31 条 migration 与 Prisma
  schema 一致；部署 29/29、备份 32/32 通过，候选已定点删除。后续记录提交只改文档，不改变已检查的 release 输入集合。
- **P5d 生产切换与人工验收**：仅在用户明确授权后连接生产，以 `platform.admin` 维护、停止 worker、验证一致备份、
  migrate deploy、原子切换、恢复服务并完成多账号/VOD/分析验收。本地 `localhost` 已补验真实 VOD 分析完成态、
  waveform/spectrogram/F0、有效窗口渐进加载、音轨范围外留空、缓存回访，以及真实管理员/教师隔离会话的在线成员、
  权限入口、任务范围和跨账号 revision 追赶。当前用户明确要求不部署生产，因此生产连接、切换与生产 origin 人工验收
  仍保持未执行；P5d 不得由滚动 goal 默认为已授权或用 localhost 证据冒充生产通过。
...
| P4 | 已完成 | P4a-P4d 路由/流、数据库/对象、worker、指标/诊断/运行手册与全矩阵回归完成；暂不部署 |
| P5 | 本地范围完成；生产待授权（P5d） | P5a-P5c 已完成候选门禁、负载验收、隔离全链路彩排、部署维护窗口复用和原子切换/回滚；最终运行候选、localhost VOD 分析与双账号在线/权限/追赶均已补验；P5d 生产连接、切换和生产人工验收仍必须由用户明确授权 |
