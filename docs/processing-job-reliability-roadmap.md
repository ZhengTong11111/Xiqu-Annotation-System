...
- **P5c 生产候选演练包（已完成）**：修正现有手册中维护排空晚于一致备份、示例遗漏 worker/备份/候选检查的危险顺序；生成
  ...
  合同矛盾：独立备份仍自行管理窗口，部署备份必须显式使用 `--require-existing-maintenance` 且不得解除外部窗口。
...
| P5 | 待用户授权（P5d） | P5a-P5c 已完成候选门禁、负载验收、隔离全链路彩排、部署维护窗口复用和原子切换/回滚；localhost VOD 分析与双账号在线/权限/追赶已补验；P5d 生产连接、切换和生产人工验收仍必须由用户明确授权 |
