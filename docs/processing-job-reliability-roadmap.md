...
- **P5d 生产切换与人工验收**：仅在用户明确授权后连接生产，以 `platform.admin` 维护、停止 worker、验证一致备份、
  migrate deploy、原子切换、恢复服务并完成多账号/VOD/分析验收。P5d 不得由滚动 goal 默认为已授权。
...
| P5 | 待用户授权（P5d） | P5a-P5c 已完成候选门禁、负载验收、隔离全链路彩排、部署维护窗口复用和原子切换/回滚；最新 `ff45b1b` 候选复核通过，localhost 双账号与分析 UI 仍需补验；P5d 生产连接和切换必须由用户明确授权 |
