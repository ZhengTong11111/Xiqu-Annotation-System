...
当前开发基线：R1 资源操作闭环、R2.2 快照安全恢复与 R2.3b1 双侧时间差异概览已完成

当前阶段：下一独立切片进入 R2.3b2 比较结果到编辑器的显式定位会话；先定义平台打开左/右文件并
携带初始 focus time 的单文件会话协议，不在比较 Dialog 内复制第二套 Timeline，也不与片段合并、
已确认标注范围或实时协作混做
...
- 恢复快照已有受控列表、只读预览和安全恢复 mutation；普通标注文件已有结构化 diff，但尚未提供
  时间轴/时间范围可视联动、快照对当前文件比较或片段合并。
...
- R2.3b2 下一步：设计“从选中差异打开左侧或右侧标注文件并定位时间”的平台会话协议。现有
  `Timeline` 是 mutation callback 很重的编辑工作站，`PlatformWorkspace` 也只承载一份活动项目；在
  b1 中不把它硬塞进比较 Dialog。b2 应通过明确的只读/可编辑文件会话和初始 focus time 复用编辑器，
  而不是复制 Timeline 或让 diff 直接操作 undo/history。
