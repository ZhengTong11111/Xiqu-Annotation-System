...
1494.4 秒后台分析与 350 项派生资产浏览器验收。视频播放已明确阻塞在阿里云 Web Aliplayer License：
服务端凭据和 PlayAuth 均正常，但账号尚未提供与测试域名匹配的 `domain + key`。代码已补齐受控 License
配置链路；取得授权后仍需完成真实播放、跳转、循环、倍率、独立窗口和凭据刷新 smoke。
...
- `MediaFile` 已能严格表达 uploaded/aliyun_vod 与 video/audio，VOD 不再伪造 `FileObject`；App 已只依赖
  统一播放控制器，原生媒体与受控 Aliplayer 后端共用精确 seek/play/pause/rate 快照语义。当前剩余问题是
  波形仍会在浏览器中下载并解码完整视频，频谱和 F0 继续依赖整段 PCM；R3h4 必须将其替换为分析音频来源、
  后台流式解码和按窗派生资产，不能复用播放 URL 作为长视频分析输入。
...
- **R3h4：波形、频谱和 F0 分析资产（已完成）。** 把“浏览器 fetch 完整视频 -> 全量 AudioContext 解码 -> 整段 PCM
  ...
  采样边界，避免跨瓦片累积时间漂移。平台长媒体不再进入浏览器全量解码路径，本地模式保留 256 MiB 上限。
  ...
  真实视频播放仍待阿里云 Web 播放器 License。Aliplayer 2.38.3 和控制台样例使用的 2.35.4 均明确返回
  `4036 LICENSE ERROR: 未配置License`，因此降级不是修复。运行配置现支持成对的 Web License domain/key，
  播放会话在缺项时提前返回稳定错误，并把有效配置传给官方 SDK；不得把 Web License 与服务端 AccessKey、
  PlayAuth 或 CDN URL 鉴权混为一谈。

  R3h4 最终回归通过媒体分析专项 16/16、完整 API 160/160、备份 28/28、部署检查 18/18 和生产构建；冷加载
  编辑器后原生视频首帧、1494.4 秒分析状态、波形、频谱及 F0 开关均可用。真实 VOD API 与纯音频分析
  已通过；Web License 配置后的真实视频播放仍是部署验收项，在完成前不得宣称该 vid 已真实播放成功。
