...
当前开发基线：R1.2 提交 `00483b1`；R1.3 递归复制已完成实现、迁移与自动化验证，待浏览器人工验收

当前阶段：R1 资源管理操作闭环进行中；下一切片为 R1.4a 原子批量移动，随后为 R1.4b 桌面拖拽移动
...
- R1.4a 下一步：建立服务端原子批量移动合同，复用目标选择器完成多选移动；单资源移动收敛到
  同一服务实现，不能保留两套事务规则。
- R1.4b 后续：在批量移动合同稳定后增加列表/网格/面包屑拖拽目标；拖拽只是桌面交互入口，
  键盘和对话框移动仍需保留，不把前端命中判断当成安全边界。
