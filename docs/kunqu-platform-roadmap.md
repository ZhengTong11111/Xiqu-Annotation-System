...
当前开发基线：R1.3 提交 `e235adf`；R1.4 原子批量移动与桌面拖拽已在工作区完成实现和自动化验证，真实拖拽已触发移动，待确认根视图修复后提交

当前阶段：R1 资源管理操作闭环进行中；下一步先完成 R1.4 最终验收与独立提交，再进入 R1.5 真正的 Finder 式 column view
...
- `pg` 8 在 Prisma adapter 的并发事务测试中会输出一条 pg 9 前置弃用警告，当前测试结果不受
  影响，升级依赖前需跟踪上游修复。
- 原子批量移动、多选目标选择器及列表/网格/面包屑拖拽已实现。真实拖拽已验证会调用移动；验收中
  发现并修复 `all_projects` 平铺嵌套项目造成的“像是复制”问题，待刷新后确认最终显示并提交。
- column view 尚不是真正的 Finder 多列导航。
- 恢复快照没有用户界面、预览和恢复操作闭环。
...
- R1.4a 已完成实现：服务端原子批量移动合同、父子选择逻辑根归一化、多选目标选择器，以及
  单项移动接口复用同一事务核心。
- R1.4b 已完成实现：列表/网格/面包屑拖拽目标；拖拽使用 Pragmatic DnD，但键盘和对话框移动
  仍是正式入口，API 继续承担权限、循环、回收站和名称冲突的最终校验。
- R1.4 验收修复：`all_projects` 已收敛为只显示顶层项目，避免资源已移动但仍从聚合视图穿透显示；
  集成测试验证数据库父级、根视图和目标目录三者一致。
- R1.5 下一步：真正的 Finder 式 column view，包括路径列模型、多列独立滚动、左右方向键导航、
  选中资源与 Inspector 同步，以及与现有选择、右键、拖拽和权限状态共用同一操作入口。
- 回收站永久删除、保留期限和对象清理策略。
- 批量删除、标签和权限操作；批量移动由 R1.4a 单独收口，复制已在 R1.3 建立统一语义。
- 所有资源操作补充审计、事务和集成测试。
...
- 恢复快照列表、创建者、时间、原因和 revision 展示。
- 快照只读预览与恢复；恢复本身生成新的 recovery snapshot 和 audit log。
- 标注文件 diff：先支持 ProjectData 的结构化摘要，再支持轨道/时间范围可视比较。
