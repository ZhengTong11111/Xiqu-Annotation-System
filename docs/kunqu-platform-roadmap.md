...
当前开发基线：R1.5 Finder 式分栏导航已提交为 `c8d8c32`；提交后发现的详细列表 class 回归已在
工作区修复并通过浏览器复核，需先独立提交该修复再开始 R1.6

当前阶段：R1 资源管理操作闭环进行中；下一独立切片为 R1.6 原子批量移入回收站，不与永久删除、标签或批量 ACL 混做
...
- R1.6 下一步：原子批量移入回收站。父项与后代同时选中时折叠为逻辑根；整批校验权限和活动树，
  在同一事务中软删除并写可解释审计。前端工具栏、右键和 Delete/Backspace 共用同一批量入口，并
  提供明确的成功/失败反馈。
- 回收站永久删除、保留期限和对象清理策略。
