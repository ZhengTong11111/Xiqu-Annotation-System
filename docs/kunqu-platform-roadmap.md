...
liveness/readiness、低基数 Prometheus 指标和管理员容量/一致性诊断。下一轮 R3d2 聚焦 PostgreSQL 与
对象目录一致备份恢复及隔离演练；`pg_trgm` 仍作为数据库级部署能力留到运维基线显式预置。
...
- R3d1 已完成：健康检查拆分 dependency-free liveness 与数据库/对象目录 readiness；每个 API 实例使用
  ...
  且保留显式确认的合格孤儿清理入口。
- R3d2 当前下一轮：设计全局维护/写入静默边界，建立 PostgreSQL 与对象目录的一致备份、校验 manifest、
  隔离恢复演练和回滚说明；为 S3/MinIO 适配明确接口和故障注入点，但不在同轮迁移存储。
- 服务端分页、排序和基础名称搜索已完成；标签、负责人、媒体类型和时间范围等高级过滤留待独立切片。
- 标签、负责人、媒体类型、更新时间等索引。
- 分片/断点续传与大于 2 GB 资产仍待 BigInt DTO/数据库整体迁移；基础上传校验、配额和孤儿生命周期
  已完成。
- PostgreSQL migration 流程已建立；一致备份恢复演练与外部告警接入仍待完成。
- 管理员诊断工具已完成；通用审计日志浏览、筛选和导出仍待完成。
