...
当前开发基线：R1.1 提交 `8ca74c3`；R1.2 回收站恢复闭环已完成实现与自动化验证

当前阶段：R1 资源管理操作闭环进行中；下一切片为 R1.3 递归复制与统一复制/粘贴语义
...
- R1.3 下一步：消除“前端可复制所有资源、后端只支持标注文件”的矛盾，完成文件夹/项目递归
  复制并确定媒体逻辑引用与物理对象复用语义。
- 后续：拖拽移动与批量移动。
