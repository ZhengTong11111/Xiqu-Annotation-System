...
适合公网媒体的波形/频谱/F0 分析管线。项目文件 v7 已完成：句级字幕在既有“念白/唱”之外，
把单值角色行当升级为项目角色列表的有序多选子集；至少选择一个合法角色才视为完成。v1-v6 文件通过
唯一迁移入口升级，历史单值 operation 只在明确 legacy parser/apply 边界兼容；旧逐字唱法字段已从当前模型和 UI 退出，由自定义腔格轨
...
v6 句级角色列表已由项目级 state 叶与受影响句内容叶组成同一结构事务；v7 已将内容叶升级为
角色数组，并继续保持配置与引用变化原子提交，不再复用已退役的内建逐字唱法
...
- “所有项目”根视图提供管理员/超级管理员专用的标注 JSON 批量导入，一次最多选择 300 份。批量窗口先按文件名
  开头连续数字精确匹配同编号顶层项目/文件夹，再在该容器内精确匹配同编号直属视频；前导零有意义，并且只有唯一
  具备 `create_child` 的目标与唯一具备 `read + download` 的视频才可提交。顶层项目、顶层文件夹及各目标的视频查询
  均使用稳定分页，每次查询最多扫描 25 页；坏 JSON、缺编号、JSON 重名、目标或视频缺失/无权/歧义以及扫描超限
  均显式停留，不根据不完整列表猜测。每个成功项调用管理员专用创建接口，服务端在事务内重读账号角色并复核现有
  目录/媒体 ACL；普通文件夹内单份 JSON 导入仍走原有接口。批量允许清楚报告部分成功，不伪装成数据库原子批次。
...
- R2.5f1 已完成：在不改写任何既有审核事实的前提下，为审核员后台排空确认与评论/反馈两条 opaque cursor
  历史流，补齐确认 keyset 复合索引、500 条大页和部分失败可见性；普通标注 revision 推进只更新审核 freshness，
  不再反复下载完整历史。Timeline 范围块增加复用既有权限/事务/审计的右键撤销或撤回入口，跨表相同原始 id
  通过类型限定身份隔离。新增带来源文件身份和版本号的完整审核包导出，导出前强制确认两类历史均已排空、属于
  同一文件且 revision 一致。本阶段没有物理删除，也没有改变记录所属文件；本地 migration 前后确认 5 条、范围
  记录 1 条保持不变，浏览器只验证撤销对话并取消，没有改动现存审核事实。
- R2.5f2 已完成：`xiqu.annotation-review-package` 现在经过唯一运行时 parser、记录上限和稳定 SHA-256 内容指纹，
  服务端逐项核对来源活动文件、读取权限、名称/revision 及数据库中的完整确认、评论和反馈事实；目标同时要求
  `read + review`、精确当前 revision、已知媒体时长和完全一致的持久轨道 id。重新链接只新增独立
  `AnnotationReviewLink` 不可变快照，使用目标+指纹唯一键、稳定加锁顺序、事务内二次验证和软撤销审计；从不更新
  原记录的 `annotation_file_id`、正文、作者、时间或生命周期。审核面板提供本地包选择、服务端 dry-run、确认关联、
  来源批次查看与批次撤销；有效关联以明确“关联确认/评论/反馈”进入 Timeline，撤销后只隐藏目标投影，来源事实仍在。
  v1 原生导出刻意不递归包含关联快照，且不做名称猜测、时间偏移或无法核验的跨服务器导入。本机 additive migration
  前后原生确认 8 条、范围记录 1 条保持不变；专项测试、完整 API 272/272、完整构建和浏览器入口验收通过。
- R2.5f2a 已完成：Timeline 审核/评论/反馈块的普通点击只拦截底层时间轴事件，不再移动播放头或横向视口；右侧审核
  面板和显式右键“定位到该范围”继续从范围起点导航。横向定位改为纯几何算法：短范围在 164px 悬浮轨道头右侧的
  可见内容区居中，宽于内容区的长范围从轨道头右侧保留 24px 余量开始显示，并在时间轴首尾 clamp。没有改动审核事实、
  播放器、吸附、拖拽、缩放或文件格式；专项测试与实际浏览器的播放头/scrollLeft 观测均已覆盖。
- R2.5f2b 已完成：循环范围块新增右键快捷菜单，保留既有左键/拖动行为并提供循环开关、从起点循环、播放一次和清除范围；
  `review` 权限显示添加确认/审核评论，`write` 权限显示添加编辑反馈。三种写入均在原位弹出紧凑输入窗口，固定使用右键
  时范围快照和“全部标注”目标，继续经过共享 dirty/revision/权限门禁与 `useAnnotationReviews` mutation，不打开右侧面板、
  不改变播放头，也没有增加 ProjectData/JSON/API 字段。专项权限测试、审核回归、完整构建和实际浏览器弹窗验收通过。
- R2.5f3 仅在出现明确业务需求后再评估：跨服务器来源需要可验证签名/信任域，非同构轨道需要用户显式映射和独立
  dry-run 合同；不得用文件名猜测、静默缩放时间或放宽 R2.5f2 的来源真实性门禁。
...
- R2.6b 已完成：项目 JSON 升级到 v7，句级 `roleType` 单值改为 `roleTypes` 多选数组。当前格式使用空数组
  表示未标注，数组必须唯一且全部引用项目角色列表；v6 合法单值迁移为单元素数组。Inspector、右侧句级列表与
  Timeline 右键将提供一致的复选交互，句子拆分继承完整集合、跨句合并取合法并集，角色重命名/删除/替换继续
  与配置 state 叶组成同一租约事务。新命令只产生数组字段，历史标量命令在有界 legacy 边界转换后重放；部分
  precondition 不匹配仍 fail closed。schema、命令、diff/merge、草稿追赶、完整测试与本地浏览器验收均已覆盖；
  大批量角色配置联动超过 500 项命令预算时明确阻止，不静默截断或退回整份 JSON 覆盖。
- R2.6c 已完成：每条句级字幕可从句级列表或 Timeline 句块右键执行“将本句逐字重置为平均时间”。系统按当前
  时间轴顺序把已有逐字块连续平均铺满句级范围，严格保留逐字 id、文字、四声、数组顺序及句级正文/分类；没有
  逐字、句范围无效或已经平均时不制造修改。关联工尺块与符号沿用父块比例映射，并与全部逐字 timing 变化进入同一
  可重放、可撤销 transaction；命令预算或完整性证明失败时明确阻止，不退回整份 JSON 覆盖。本轮不改变项目文件版本。
  确认窗口支持“本次打开文件不再提示”，该偏好只存在当前编辑器内存会话，取消不会启用，重新打开或刷新后恢复；
  即使窗口停留期间项目发生变化，确认时也会从最新 ProjectData 重新校验并构造事务。
...
- R4c4 已完成代码与自动化回归：平台编辑器默认在连续 3 次完整保存失败后，于最近所属项目的 `backup`
  ...
  API 1/1、自动保存 9/9、原子提交 28/28、草稿 37/37 与完整构建通过；偏好开关、3/5/10 次阈值和当前失败
  周期统一位于编辑器“文件”菜单，不再混入“视图”菜单。真实浏览器失败注入及生产部署待验收。
...
- R5a4c4 已按风险与依赖闭包拆分，不能用一个宽松“轨道更新”快照同时覆盖稳定配置和大规模生命周期：
  - R5a4c4a 已完成：新增轨道顺序、既有内建轨配置、既有附属点轨配置三类有界结构叶命令；内建
    ...
    state 目标；v7 已改为与受影响句子的 `roleTypes` 数组内容叶组成同一结构事务。
...
> run/artifact schema 与身份、D2b 创建/复用/需求、D2c worker 原子对象发布、D2d 应用与 operation 绑定。D2a-D2c 已完成：
...
> 模糊提交补偿和受保护读取均已落地。执行开关继续默认关闭，只有真实外部执行器绝对路径与请求开关同时配置才启用；当前进入
> D2d 预测应用与真实 operation/revision 绑定。
> 行为旁表使用独立
