...
当前阶段：R2 已完成恢复、文件比较、选择性整合和“已确认标注范围”端到端审核闭环。R2.5c 已把
真实确认 API 接入平台编辑器 Inspector 与只读时间轴栏，并以 dirty、服务器 revision 和 `review` ACL
约束创建/撤销；确认事实继续独立于恢复快照、选择性整合和 ProjectData。下一阶段进入 R3a，先稳定
大规模资源列表的分页、排序、搜索合同与数据库索引，再接前端虚拟列表。
...
- R3a 当前下一轮：先定义统一的资源查询合同和 PostgreSQL 查询/索引基线，覆盖真实目录与聚合视图的
  cursor 分页、稳定排序、名称搜索及必要筛选；保证 ACL、回收站祖先隐藏和 `all_projects` 顶层语义
  不因分页而退化。前端先增加可增量消费的状态边界，本轮不同时重写所有三种视图或引入虚拟列表。
