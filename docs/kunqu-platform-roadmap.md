...
当前开发基线：R1 资源操作闭环、R2.2 快照安全恢复与 R2.3b2 差异到编辑器的单侧定位已完成

当前阶段：下一独立切片进入 R2.3c 选择性片段整合的业务合同设计；先界定目标文件、实体引用、
时间范围、冲突决策、权限与单次历史提交，不把合并做成整份 JSON 覆盖，也不与已确认标注范围或
实时协作混做
...
- 恢复快照已有受控列表、只读预览和安全恢复 mutation；普通标注文件已有结构化 diff、双侧时间概览
  和按侧打开定位，但尚未提供快照对当前文件比较或选择性片段合并。
...
- R2.3c 下一步：设计从两个标注文件选择实体/时间片段整合到明确目标文件的合同。应先定义可合并
  领域、跨实体引用、目标权限、冲突预览与用户决策，并保证整合在目标编辑器中形成一次可撤销提交；
  不得直接整份覆盖，也不能把恢复快照当作发布版本。
