...
当前阶段：R2 已完成恢复、文件比较、选择性整合和确认范围审核闭环；R3a 已完成资源查询的稳定 opaque
cursor、数据库总序、有限候选扫描、ACL 后填页、目录查询索引与 list/grid 增量加载。下一阶段进入
R3b：为 Finder column 建立逐列分页并为 list/grid/column 接入共享虚拟化，保证 1000+ 已加载资源下的
DOM、选择、拖拽和键盘行为可控。`pg_trgm` 作为数据库级部署能力留到运维基线显式预置。
...
- R3b 当前下一轮：让 Finder column 的每一逻辑列独立持有 nextCursor/loading/error，并以成熟、headless
  虚拟化依赖统一 list/grid/column 的大列表渲染。必须保持逻辑列选择、Shift/Cmd 多选、右键、拖拽、
  路径截断和错误保守规则；目标选择器先做增量目录加载，不得偷偷拉取全集。
