...
> 取消/重试重放、8 轮 worker 启停压力，以及管理员任务中心、诊断和 localhost VOD 播放/随机跳转验收。第二个普通账号
> 浏览器会话未登录的人工缺口已明确保留，权限、共享执行和协作核心由隔离数据库集成测试覆盖。下一阶段 P5c 准备脱敏
> 生产候选演练包。P5c 已修正维护晚于一致备份的危险顺序，新增绑定期望旧版本的原子 `release:switch`、脱敏切换/
...
> `--require-existing-maintenance` 复用且不解除窗口。修复后的 `ff45b1b` 又重新组装为完整候选并通过 27 个运行路径、
> 25 个生产依赖、31 条 migration 与 Prisma schema 门禁；localhost 双账号和分析 UI 仍保留为人工缺口。P0-P5c 均按用户要求暂不部署；
> 下一阶段 P5d 必须重新取得生产连接、维护、备份、部署和人工验收的明确授权。后续每阶段必须独立
> 测试、文档化和提交，不能把
