...
R3h1/R3h2 已于 2026-08-05 完成并通过浏览器、专项测试、完整 API 测试和生产构建验收；当前立即阶段为
R3h3 统一播放控制器，尚未进入分析资产实现。
...
- `MediaFile` 已能严格表达 uploaded/aliyun_vod 与 video/audio，VOD 不再伪造 `FileObject`；但
  `VideoPlayer` 与 App 仍直接依赖 `HTMLVideoElement`，波形还会在浏览器中下载并解码完整视频，频谱和
  F0 继续依赖整段 PCM。这套管线尚不适合短时签名、HLS、跨域或长视频，按 R3h3-R3h4 继续替换，不能
  只嵌入第三方播放器脚本。
