...
当前开发基线：R0 提交 `fc5471a`；R1.1 单资源移动目标选择器已完成实现与验证，尚待形成独立提交

当前阶段：R1 资源管理操作闭环进行中；下一切片为 R1.2 回收站恢复与虚拟视图操作语义闭环
...
- 标注 JSON 导入、标注文件复制、打开编辑和 revision 保存。
- 保存前恢复快照与并发冲突检查。
...
- `pg` 8 在 Prisma adapter 的并发事务测试中会输出一条 pg 9 前置弃用警告，当前测试结果不受
  影响，升级依赖前需跟踪上游修复。
- 单资源移动已有专业目标选择器；拖拽/批量移动和目录递归复制尚未完成。
...
- R1.2 下一步：回收站恢复入口、批量恢复反馈、虚拟视图上下文操作，以及恢复到有效资源树的
  服务端不变量。
- 后续：拖拽移动与批量移动。
- 文件夹/项目递归复制；定义媒体引用与物理复制语义。
- 真正的 column view，多列独立滚动与键盘导航。
- 回收站批量恢复/永久删除策略。
- 批量移动、复制、删除、标签和权限操作。
- 所有资源操作补充审计、事务和集成测试。
...
- baseline migration 后是否立即引入正式 migration 发布规范。
- 目录递归复制时媒体采用引用、按需物理复制还是策略可选。
