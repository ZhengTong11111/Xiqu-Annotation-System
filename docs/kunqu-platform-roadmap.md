...
当前开发基线：R1 资源操作闭环、R2.2 快照安全恢复、R2.3b2 差异到编辑器的单侧定位，以及
R2.3c1 选择性整合的引用依赖计划核心已完成

当前阶段：等待确认后进入 R2.3c2 选择性整合的比较端选择与预检 UI；只展示用户选择、自动依赖、
目标动作和结构错误，不修改目标文件。R2.3c3 再单独实现冲突决策和目标编辑器中的一次可撤销提交，
不把整合做成整份 JSON 覆盖，也不与已确认标注范围或实时协作混做
...
- 选择性整合已有纯计划核心：以结构化 diff 稳定 key 和明确方向计算 source/target，自动闭包句级、
  逐字、工尺谱、板眼、自定义轨道/递归父块和附属打点的强引用，并区分新增、冲突与目标已相同；
  当前尚未接入可见选择 UI，也不会修改任何项目。
...
- 恢复快照已有受控列表、只读预览和安全恢复 mutation；普通标注文件已有结构化 diff、双侧时间概览、
  按侧打开定位和选择性整合的依赖计划核心，但尚未提供快照对当前文件比较、选择/预检 UI 或实际
  目标文件整合。
...
- R2.3c2 待确认：在现有比较 Dialog 增加方向、实体多选和依赖计划预览。UI 必须完全消费 c1 输出，
  清楚展示自动加入的依赖、目标已相同、待决冲突和不可执行原因；筛选只影响可见列表，不能悄悄取消
  已选实体。本阶段仍只读，不进入编辑器、不保存目标文件。
- R2.3c3 后续：为每个 replace-conflict 提供保留目标/采用来源的明确决策，并把已确认计划交给目标
  单文件编辑器，形成一次可撤销 `commitProject()`。应用前必须重新读取目标最新 revision/权限并
  重新预检，禁止整份覆盖、陈旧 payload 写入或把恢复快照当作发布版本。
