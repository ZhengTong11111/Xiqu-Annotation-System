...
维护模式与写入静默边界。下一轮 R3d2b 在该边界上实现 PostgreSQL 与对象目录一致备份、manifest 校验
和隔离恢复演练；`pg_trgm` 仍作为数据库级部署能力留到运维基线显式预置。
...
- R3d2b 当前下一轮：在维护静默边界上建立 PostgreSQL 与对象目录的一致备份、校验 manifest、隔离恢复
  演练和回滚说明；提供不依赖浏览器 session 的受控 CLI 维护恢复通道，并为 S3/MinIO 适配明确接口和
  故障注入点，但不在同轮迁移存储。
- 服务端分页、排序和基础名称搜索已完成；标签、负责人、媒体类型和时间范围等高级过滤留待独立切片。
- 标签、负责人、媒体类型、更新时间等索引。
- 分片/断点续传与大于 2 GB 资产仍待 BigInt DTO/数据库整体迁移；基础上传校验、配额和孤儿生命周期
  已完成。
- PostgreSQL migration 和维护静默流程已建立；一致备份恢复演练与外部告警接入仍待完成。
