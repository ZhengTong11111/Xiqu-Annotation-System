...
- R2.5f2 待推进：为“重新链接到文件”设计来源不可变、目标可撤销的新增关联模型与 dry-run。不得通过更新
  `annotation_file_id` 移动记录；目标权限、来源身份、revision/轨道映射、重复导入和事务补偿完成验证前，
  不开放写入入口。任何实现都必须证明源确认、评论和反馈的数量、正文、作者、时间与生命周期保持不变。
