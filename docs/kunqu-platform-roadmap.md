...
当前开发基线：R1 资源操作闭环、R2.2 快照安全恢复与 R2.3a 文件结构化比较已完成

当前阶段：下一独立切片进入 R2.3b 时间范围可视比较；以已完成的纯 ProjectData diff 为数据源，
先定义可定位差异与现有时间轴的只读联动边界，不与片段合并、已确认标注范围或实时协作混做
...
- R2.3b 下一步：时间范围可视比较。先把结构化差异转换为可筛选、可定位的时间范围导航，并设计
  与现有 Timeline 的只读联动；不得为此复制第二套 ProjectData normalization、修改文件或污染编辑器
  undo/history。是否采用双时间轴或单时间轴差异标记，应在读取现有窗口/时间轴装配代码和真实大文件
  性能后再确定，而不是预先固定视觉方案。
