...
签名校验、并发安全容量边界、跨文件系统/数据库补偿和管理员对象孤儿审计。下一轮 R3d 转向结构化
运行指标、备份恢复演练与管理员诊断；`pg_trgm` 仍作为数据库级部署能力留到运维基线显式预置。
...
- R3d 当前下一轮：建立结构化健康/容量/上传失败指标和管理员诊断摘要，制定 PostgreSQL 与对象目录的
  一致备份/恢复步骤并在隔离环境演练；为 S3/MinIO 适配明确接口和故障注入点，但不在同轮迁移存储。
