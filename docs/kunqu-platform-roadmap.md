...
当前开发基线：R1 资源操作闭环、R2.2 快照安全恢复，以及 R2.3 普通标注文件的结构化比较、
单侧定位和选择性整合闭环已完成

当前阶段：进入 R2.4 恢复快照与当前文件的只读结构化比较。快照仍是事故恢复历史，不是普通文件或
发布版本；本阶段只复用现有迁移、diff 和恢复详情权限边界，不把快照直接送入选择性整合或另建迁移
路径
...
- 恢复快照已有受控列表、只读预览和安全恢复 mutation；普通标注文件已有结构化 diff、双侧时间概览、
  按侧打开定位和选择性整合闭环，但尚未提供快照对当前文件的结构化比较。
...
- R2.3c3 已完成：每个 replace-conflict 必须显式选择保留目标或采用来源。准备应用时重新读取两侧
  ...
  与 409 保护。
- R2.4 下一步：在标注文件 Inspector 的恢复历史中提供“与当前文件比较”。当前 payload 与所选快照
  详情都必须按需读取并经 `normalizeImportedProjectFile()` 进入现有 `annotationDiff`；复用双侧时间
  概览与差异列表，但快照侧保持只读，不进入选择性整合，也不改变恢复 mutation 的二次确认语义。
- R2.5 后续：设计“已确认标注范围”作为独立、可审计的研究审核层，而不是 ACL、普通快照或文件内
