...
> 文档状态：专项实施中，RA0-RA2、RA3 代码门禁、RA4a-RA4c2 及 RA5a 代码已完成；RA3 多环境听觉验收延期，当前推进 RA5b
...
- 下一阶段是 **RA5b 漂移、缓冲诊断与受控恢复**。public/生产尚未部署 RA4c2；RA6 rollout 前必须先部署
...
#### RA5b：漂移、缓冲诊断与受控恢复（下一阶段）

**改动范围**

- 审查既有漂移采样、软/硬同步阈值和连续样本门禁，避免重复建立播放器状态机；
- 为同步决策、缓冲进入/恢复和会话代际增加有界、脱敏、低频诊断事实；
- 补齐慢网、迟到回调、频繁 seek 和短暂抖动不会触发错误硬同步的自动测试；
- 只在证据证明有缺口时修改恢复策略，保持视频为唯一主时钟。
