...
- 按用户本轮明确指示不创建部署前备份；已经启动的备份 CLI 在命令启动后立即收到中止信号并返回
...
## 2026-08-30：生产本地备份保留清理

- 清理前只读盘点确认 `/var/lib/xiqu-platform/backups` 存在 15 份完整本地备份，且每份都会独立保存数据库 dump
  和整个对象快照，不是增量链；旧备份因此包含大量重复对象。此前部署中止的备份命令没有留下新备份目录。
- 删除前使用当前 release 的 `backup:verify` 完整校验两份关键恢复点：
  `xiqu-backup-2026-08-26T07-12-24-634Z-7b3ff9d9` 与
  `xiqu-backup-2026-08-30T20-15-16-936Z-a88d7792` 均返回 `valid: true`、零错误。
- 经用户明确确认后，按完整目录名删除 11 份重复旧备份；删除命令先验证 11 个目标全部是预期目录，再逐目录清理，
  没有使用覆盖整个备份根的通配删除。
- 当前保留首次生产基线（8 月 6 日）、阶段恢复点（8 月 16 日）、RA6 迁移前恢复点及迁移报告（8 月 26 日）、
  最新完整恢复点（8 月 30 日）。数据盘使用量从 52 GB 降至 26 GB，剩余空间从 42 GB 增至 68 GB，使用率
  从 56% 降至 27%。
- 清理过程不需要维护窗口；完成后 `xiqu-api`、`xiqu-analysis-worker` 均为 `active`，维护状态保持关闭。
...
- **待人工验收**：使用真实 v6 JSON 导入后选择三人以上合说并导出 v7；在双账号平台会话中分别修改同一句角色，
  核对后提交端协调、undo/redo 和自动保存。
- **主线集成**：功能提交随后与远端 `main` 的账号并发治理、ACL 事务复核、分析 worker fencing、日志脱敏和批量导入
  提交合并；合并后的 API 集成测试 270/270、完整构建和 `git diff --check` 均通过。代码已进入主线，但本轮没有部署
  生产服务器。

## 2026-08-31：所有项目按编号批量导入标注 JSON

### 匹配与导入规则

- “所有项目”根视图的标注 JSON 入口现在只向 `admin`/`super_admin` 开放，可一次选择最多 300 份 JSON；普通账号
  的根入口不可执行。进入具体项目/文件夹后的单份 JSON 导入仍保留原有跨目录媒体选择器，不被批量权限收紧。
- 新增纯 `annotationBatchImport.ts` 两阶段规划：从 JSON 名最开头提取连续 ASCII 数字，先精确匹配同编号顶层项目或
  文件夹，再在该目标内精确匹配同编号直属视频。前导零被保留，因此 `001` 不会误绑 `01` 或 `0010`；只有唯一且
  具备 `create_child` 的目标、唯一且具备 `read + download` 的视频才可导入。
- 顶层项目和顶层文件夹分别使用稳定 cursor 完整扫描；只为唯一可写目标查询视频，查询并发上限为 6。每次查询
  最多读取 25 页，一次最多处理 300 份 JSON；扫描超限 fail closed。坏 JSON、缺编号、JSON 同名、目标或视频
  缺失/无权/歧义均保留为明确未导入行。
- `BatchAnnotationImportDialog` 展示 JSON、编号、目标项目/文件夹、关联视频和逐项状态，并逐项调用新的
  `/api/annotation-files/batch-import-item`。该接口先拒绝非管理员，再在资源树事务内重读当前账号角色，同时复核目录
  `create_child`、名称冲突与媒体权限，避免角色撤销后的旧会话绕过。普通 `/api/annotation-files` 合同保持不变。
  单项失败不阻断后续文件，成功项不重复提交，完成后通过 `refreshCurrentView()` 刷新根视图；流程允许部分成功，
  不伪装成数据库原子批次。

### 验证

- `npm run test:annotation-batch-import` 通过 6/6，覆盖前导零精确匹配、顶层目标和目标内视频的缺失/无权/歧义、
  分页过滤/去重与扫描上限拒绝。
- API 集成测试覆盖普通账号调用批量专用接口被拒绝、其普通单份导入仍成功，以及管理员调用专用接口成功。
- 完整 `npm run build` 通过 shared、document-model、Prisma Client/schema guard、Web 和 API；仅保留既有 Vite 主
  chunk 超过 500 kB 提示。`git diff --check` 通过。
...
## 2026-08-31：确认列表 200 条上限即时修复

### 生产证据与修改边界

- 生产文件 `010_zuili_zyx_v001_annotation.json` 当前有 286 条确认记录。4:42–11:26 的 52 条记录全部仍在
  `annotation_confirmations`，均绑定 revision 3699；其中 51 条 active、1 条因重复发送撤销，7 条带确认备注。
  revision 3699 的恢复快照也仍存在，没有执行恢复、覆盖或删除。
- 消失原因是 `listAnnotationConfirmations()` 固定只按创建时间倒序读取最新 200 条：接口最早返回时间为
  2026-08-31 15:13:34，而目标范围的 52 条全部位于被静默遗漏的 86 条中。范围评论表为 0 条；用户看到的文字
  当时属于确认备注，不是后来新增的独立 `review_comment`。
- 即时修复仅把确认列表查询上限集中为命名常量并从 200 提高到 1000。排序、`read` 权限、活动资源门禁、DTO、
  confirmation 生命周期、ProjectData、revision、恢复快照和数据库 schema 均不改变。当前生产最大 286 条，响应
  增量可控；超过 1000 条的长期正确方案仍是游标分页，不能继续无限放大单次响应。
...
## 2026-09-01：确认历史正式分页与时间轴 Shift 横移

### 从临时容量修复迁移到稳定分页

- 用户确认上一轮 1000 条即时上限已恢复目标确认记录后，本轮删除该临时常量，正式为确认历史建立绑定
  `annotationFileId + createdAt + id` 的 opaque keyset cursor。接口默认读取 50 条、最大 100 条，按
  `(createdAt desc, id desc)` 获取 `limit + 1` 并返回 `nextCursor`；损坏、超长或跨文件 cursor 均返回 400。
  撤销事实、`read` 权限、活动文件门禁、current revision、DTO 脱敏和治理语义均保持不变，未修改数据库 schema。
- `useAnnotationReviews` 继续作为确认、审核评论和编辑反馈的唯一前端状态边界，但确认与评论/反馈分别持有 cursor、
  加载锁和继续加载入口。续页按稳定 id 去重；刷新、换文件或 mutation 后重新读取第一页，并使旧代次的成功或失败
  响应失效，避免把旧页、旧错误混入新会话。面板只报告“已加载”数量，分别提供“加载更多确认”和“加载更多评论与
  反馈”，不再把当前局部集合伪装成服务器总数。时间轴仍只渲染当前已加载记录，继续加载后自然补齐。
- 没有复制评论分页器或另建第二套审核面板：确认 cursor 独立放在 API 边界，前端两类记录共用一个按 id 去重 helper；
  旧固定上限常量、评论专用重复合并函数和含糊的单个“加载更多记录”入口已经清理。

### 时间轴横向滚轮合同

- 保留既有 `Option/Alt + 滚轮` 指针中心缩放和 `Ctrl + 滚轮/触摸板捏合` 缩放；新增常见桌面手势
  `Shift + 滚轮` 横向移动时间轴。无修饰键时不阻断浏览器，因此触摸板原生横移和轨道纵向滚动保持原样。
- 横移 helper 取 `deltaX/deltaY` 的主导轴，兼容浏览器已把 Shift 纵轮预转换为横向 delta 的情况；像素、行、页
  三种 `deltaMode` 分别按原值、40 像素/行和 90% 可视宽换算。直接修改 `scrollLeft`，不增加会滞后于精确标注操作的
  平滑动画，也没有触碰缩放、吸附、拖拽或块创建逻辑。复杂换算旁已补中文功能注释。

### 验证、自审与阶段状态

- cursor/分页纯测试 3/3，前端续页去重 1/1，滚轮换算 2/2；标注确认领域测试 14/14、确认视图测试 7/7、完整 API
  回归 271/271，shared、Web 与 API 构建均通过。API 集成测试覆盖 limit=1 两页无重复、稳定集合、末页无 cursor、
  跨文件 cursor 与非法 limit 拒绝。
- 应用内浏览器已打开本地标注工作台并确认时间轴、句级字幕、逐字轨与属性面板布局正常。浏览器控制层未能把
  Shift 修饰键与滚轮可靠合成为同一个原生 wheel 事件，实际滚动仍被浏览器当作普通纵向滚动；因此手势行为以纯
  helper 测试和 React 事件分支验证为本轮自动化证据，仍需人工用实体鼠标/触摸板做一次手感验收。
- 安全与可维护性自审确认：响应新增字段为 additive；旧前端仍可读取第一页，新前端不解析 cursor；没有新增依赖、
  数据迁移、权限旁路、完整 payload 日志或确认/评论重复状态机。`AGENTS.md` 与 roadmap 已同步长期分页合同。
  **已完成**：代码、专项/API/构建测试、浏览器布局检查、僵尸逻辑清理、文档更新与 Git 同步。
  **待人工验收**：用实体鼠标确认 `Shift + 滚轮` 横移方向和速度，并在超过 100 条确认/评论的文件中分别继续加载。
  本轮未收到生产部署指令，不切换服务器 release。

## 2026-09-01：审核历史完整视图、时间轴无损治理与导出

### 数据保护边界与问题扫描

- 本轮把“现有审核绝不能丢失”设为最高门禁：没有新增物理删除 API，没有更新确认或范围记录的
  `annotation_file_id`，没有重建事实表，也没有让导入/导出覆盖原 id。Timeline 中面向用户的删除语义严格落到既有
  幂等“撤销确认 / 撤回审核评论 / 撤回编辑反馈”，原正文、作者、创建时间、来源文件与生命周期继续保存在数据库。
- 审查发现确认与评论/反馈虽然已有独立 opaque cursor，但审核员需要反复点击两个继续加载入口；第一页任一请求失败
  还会隐藏另一类成功结果。另一个性能问题是普通标注保存和远端 revision 追赶也会刷新审核列表，审核员随后再次
  排空全部历史，导致记录较多时产生与审核事实无关的重复请求。
- 扫描数据库和 API 后确认没有确认/评论物理 delete 路径。跨表使用裸 UUID 作为 Timeline 身份存在理论碰撞风险；
  现在改为 `confirmation:<id>` / `range-record:<id>`，同时保留原 record id 和严格来源类型供右键权限解析。
- “重新链接到文件”没有仓促实现为危险移动。本轮导出包先保存可验证来源身份；后续 R2.5f2 只能新增可撤销的
  source-to-target 关联并执行权限、revision/轨道映射、重复包和事务补偿 dry-run，禁止改写源记录归属。

### 完整加载、性能和导出

- shared 新增审核历史大页上限 500；确认和范围记录路由/服务层共同使用该边界，普通首屏仍读 100 条。专项自审曾
  发现一次常量误套到项目权限管理分页且范围记录仍被 100 截断，已在提交前纠正：项目权限管理恢复原 100，只有
  两条审核历史允许 500。API 集成测试同时覆盖 500 成功与 501 拒绝。
- `useAnnotationReviews.loadAllRecords()` 并行排空确认、评论/反馈两条流，每条流内部严格串行消费自己的 opaque
  cursor；重复 cursor、超过有界请求数、文件切换、刷新代次变化或任一请求失败都会明确停止，成功流仍可见但不会
  被伪装成完整导出。首屏改用 `Promise.allSettled`，一类故障不再遮住另一类历史。
- 具备审核权限的会话在首屏后自动排空一次；同一文件的重复“加载全部/导出”触发复用一个 in-flight Promise。
  普通标注 revision 变化现在只原地推进两类列表的 current revision 以更新 freshness 和导出来源，只有审核 mutation、
  `annotation.review.changed` 或人工刷新才重读审核事实，避免每次自动保存都全量下载历史。
- 新增版本化 `xiqu.annotation-review-package` v1：包含导出时间、来源文件 id/名称/revision、完整 confirmations 和
  range records；不含 cursor、token、权限快照、媒体 URL 或 ProjectData。两条流未排空、revision 不同或混入其他
  文件记录时 fail closed，不生成看似完整的残缺 JSON。
- Prisma 只新增确认表 `(annotation_file_id, created_at, id)` 复合查询索引，保留旧索引且不含 DML。开发数据库应用
  migration 前后均为确认 5 条、范围记录 1 条，新索引存在；隔离测试 schema 也成功应用同一 migration。

### Timeline、验证与阶段状态

- Timeline 审核块右键现在提供定位及权限允许的撤销/撤回入口；右键不移动播放头、不触发框选。面板与时间轴复用
  `AnnotationReviewWithdrawalDialog`，对话框明确说明操作不会删除历史。只读账号只看到查看提示，最终权限仍由
  服务端事务重验。
- 领域与视图测试通过 14/14、审核视图/分页/排空/导出联合测试 16/16；完整 `npm run test:api` 271/271 通过，覆盖
  分页、跨文件 cursor、撤销/撤回权限与幂等、复制不继承审核事实及并发 revision 原子性。完整 `npm run build`、
  Web/API 类型检查和 `git diff --check` 通过，仅保留既有 Vite 主 chunk 超过 500 kB 提示。
- 本机浏览器以教师账号打开 `新工尺_央视_顾卫英《寻梦》.merged.cleaned.json`，一次显示 6 条审核事实；确认与评论
  Timeline 块右键分别出现“撤销确认”“撤回审核评论”，打开对话框后取消，数据库未发生 mutation；完整审核包导出
  显示成功且明确提示服务器记录未修改。使用 `127.0.0.1` 时控制台只有既有阿里云 Web License 域名不匹配错误，
  审核接口无 error。
- **已完成**：完整历史、性能边界、无损导出、Timeline 右键软撤销、索引 migration、专项/API/构建/浏览器验证、
  僵尸与物理删除路径扫描、AGENTS/roadmap/log 更新。**待推进**：R2.5f2 安全重新链接模型及 dry-run；本轮没有生产
  部署指令，不切换服务器 release。

## 2026-09-01：R2.5f2 审核包安全重新链接与来源保护

### 来源不可变合同与数据模型

- 本轮延续“现有审核绝不能丢失”的硬约束，没有移动、覆盖或删除任何 `annotation_confirmations`、
  `annotation_range_comments`。重新链接不修改源记录 `annotation_file_id`，而是新增独立 `AnnotationReviewLink`：保存
  目标文件、可空的活动来源关系、来源 id/名称/revision 快照、规范化审核包、SHA-256 内容指纹、记录数量、创建者和
  软撤销生命周期。来源以后被删除时只断开实时外键，来源身份快照仍保留；目标删除才级联其治理附属关联。
- 两条 additive migration 只增加审计枚举、关联表、索引、外键和完整性 CHECK，没有重建或 DML 原生审核表。第二条
  migration 单独追加，避免修改已经在本机执行的 migration checksum；数据库约束保证 revision 为正、总记录数
  1–1000、指纹为 64 位小写十六进制，以及撤销时间/主体成组出现。
- 本机 public schema 执行前原生确认 8 条、范围记录 1 条；执行后仍分别为 8 条、1 条，新关联表为 0 条。创建、重复
  导入、撤销和重复撤销的 API 集成测试还逐项序列化对比来源两张表，确认正文、作者、时间、revision 与生命周期均
  未改变。

### 解析、服务端核验与事务语义

- document-model 新增唯一审核包运行时 parser，严格接受 format/version、有效 ISO 时间、来源身份、精确 counts、
  完整生命周期、同源记录、无重复类型身份和最多 1000 条事实；空包也明确拒绝。解析边界会裁剪用户摘要中的额外
  roles，稳定指纹排除 `exportedAt` 但保留来源、revision 和全部事实，因此相同事实反复导出不能制造重复关联。
- dry-run 与 create 共用同一完整核验：操作者必须具备来源 `read` 和目标 `read + review`；来源必须仍为活动文件，
  名称/revision 及每一条确认、评论、反馈都与数据库当前值完全一致；目标必须保持请求 revision、具有已知正媒体
  时长，并包含审核包引用的全部真实顶层持久轨道 id。范围越界、轨道缺失、权限变化、来源篡改或 revision 漂移均
  fail closed，不按轨道名称猜测，不接受无法在本平台核验的跨服务器包。
- create 在 Serializable 事务中按稳定资源 id 顺序锁定来源和目标，再执行全部二次核验；目标+内容指纹唯一键阻止
  并发或模糊重试重复写入。每个目标最多保留 20 个有界完整批次。撤销要求目标 review，并仅允许创建者、owner
  authority 或管理员治理；重复撤销幂等返回第一次撤销事实，不覆盖原因也不重复审计。审计只记录关联 id、来源/
  目标、指纹短前缀和数量，不复制正文、完整包或凭据。

### 前端、Timeline 与验证

- 当前目标文件的“标注审核与反馈”面板新增“导入并重新链接审核包”。窗口按“读取本地 JSON -> 服务端 dry-run ->
  用户确认”三步执行，展示来源/目标、两侧 revision、记录数和精确轨道匹配；客户端解析只用于快速反馈，不能替代
  服务端授权。关联批次与原生历史分组显示，展开后可按范围定位；批次撤销复用软撤销对话框并明确来源事实不会删除。
- 有效关联事实以 `link:<linkId>:<type>:<sourceId>` 类型限定身份进入 Timeline，显示“关联确认/评论/反馈”和来源文件；
  跨文件 revision 不做虚假新鲜度比较。已撤销批次不进入 Timeline，且关联事实不能调用原生撤销 API。v1 导出继续
  只包含当前文件原生事实，不递归打包关联快照，防止 A→B→C 来源链膨胀。
- 测试通过：审核包 parser 3/3、关联 API 1/1、审核视图/分页/导出 17/17、完整 API 272/272、完整 `npm run build`
  与 `git diff --check`。API 用例覆盖无 review、目标 revision 漂移、客户端正文篡改、重复导入、软撤销和重复撤销；
  浏览器以本机教师会话打开实际编辑器，确认审核面板、导入按钮和预检弹窗均正常显示。没有实际上传用户文件或修改
  现存研究审核事实；日志中仅见先前 Vite HMR 触发的 Aliplayer 旧错误，本轮交互没有新增审核接口错误。
- **已完成**：不可变交换合同、来源逐项核验、目标门禁、additive schema、事务关联/撤销、审计、面板和 Timeline、
  完整测试与规范文档。**待按需评估**：只有出现真实需求时才设计带签名信任域的跨服务器包，或为非同构轨道增加
  用户显式映射；本轮不以模糊匹配或放宽来源核验换取“导入成功”。未收到生产部署指令，不切换服务器 release。

## 2026-09-01：R2.6c 单句逐字时间平均重置

### 领域算法与数据保护

- 新增共享 `resetSentenceCharactersToEvenTiming()`：按当前时间轴顺序将某句已有 N 个逐字块连续平均铺入句级
  `[startTime, endTime]`。排序使用开始时间、结束时间和原数组位置作为稳定 tie-break，重叠块也能得到确定结果；
  第一块精确从句首开始，最后一块精确在句尾结束，避免浮点累计留下微小缝隙。
- 该操作不是“重新拆字”：不创建或删除逐字块，不改变 id、文字、四声及其他内容字段，不重排持久数组，也不修改
  句级正文、发声方式、角色行当或句级范围，因此不需要升级项目 JSON 版本。句子不存在、没有逐字、句级范围无效或
  已经是相同平均结果时明确停止，no-op 不产生历史、待提交 operation 或服务器 revision。

### 协作命令与界面

- App 从最新 `projectRef` 计算预览，先向用户说明会覆盖本句逐字时间但保留内容；确认后复用现有
  `synchronizeGongcheWithChangedParents()`，按父逐字块比例同步关联工尺块和内部符号。全部逐字 timing、工尺块 timing
  与工尺符号 state 进入一个严格 `annotation.transaction`；builder 无法证明完整 next ProjectData 或超过命令预算时
  阻止提交，不退回 legacy/full-snapshot 保存，避免协作端只收到半截时间变化。
- 句级字幕列表的每句 Radix 右键菜单和 Timeline 句块右键菜单都增加“将本句逐字重置为平均时间...”，两处共用同一
  App 动作、只读门禁、远端 revision 追赶门禁和确认文案，没有复制第二套修改逻辑。

### 验证与状态

- 新增领域测试 3/3，覆盖三块平均分配、重叠稳定顺序、单块铺满、重复执行 no-op、逐字四声和外句保持，以及缺句/
  无逐字/坏范围；现有 transaction 8/8、timing builder 4/4、timing apply/undo 3/3 均通过。
- 本机浏览器以教师会话打开 90 句的实际示例文件，确认句级列表与 Timeline 句块两个右键入口均正常显示，菜单布局
  未破坏，控制台没有新增错误。浏览器控制层不能安全接管原生 confirm，因此没有代用户确认并修改现存服务器研究
  文件；最终行为由纯函数和可逆事务测试覆盖。
- **已完成**：共享算法、两处 UI 入口、工尺级联、严格协作命令、中文注释、专项测试、roadmap/AGENTS/log 更新。
  **待人工体验**：在非生产副本上确认一次重置与撤销的时间轴手感。本轮不部署生产服务器。

## 2026-09-01：R2.6c1 单句平均时间重置的会话内免提示

### 交互与会话边界

- 原入口使用浏览器原生 `window.confirm`，无法放置独立复选项。本轮新增 Radix AlertDialog 应用内窗口，继续明确展示
  目标句、逐字块数量、保留字段、关联工尺同步和可撤销性，并增加“本次打开文件不再提示”。复选项只有在用户点击
  “确认重置”时才生效；取消、Escape 或点击遮罩关闭均不会改变后续行为。
- 免提示状态只保存在当前 `EditorWorkbench` 的内存 ref。编辑器按平台 annotation file id 或本地会话 id 挂载，离开
  编辑器、重新打开文件、打开另一文件或刷新页面都会销毁该状态并恢复提示。没有增加 ProjectData/JSON 字段，也没有
  写入 localStorage、IndexedDB、账号偏好、服务器或协作命令。
- App 仍是唯一业务编排边界：第一次请求只把句 id、显示文本和逐字数量交给窗口；用户确认时再次读取最新
  `projectRef`，重新执行权限/追赶门禁、句范围与逐字校验、平均算法、工尺级联及 transaction builder。这样弹窗停留
  期间的本地或远端变化不会让旧预览覆盖新内容；启用免提示后也只跳过二次确认，不跳过无逐字、坏范围、no-op、
  命令预算或完整性错误反馈。

### 清理、自审与验证

- 删除该动作对 `window.confirm` 的依赖，没有复制平均算法、工尺同步或提交路径；新增组件只负责可访问的提示 UI 和
  当前复选框值。样式复用现有平台确认窗口、遮罩与按钮，仅增加说明和复选行，未引入新依赖或持久化状态机。
- `test:sentence-character-timing` 3/3、`test:annotation-transaction-command` 8/8、`test:timeline-timing-command` 4/4、
  `test:timeline-timing-command-apply` 3/3 均通过；完整 `npm run build` 和 `git diff --check` 通过，只有既有 Vite 主 chunk
  大小提示。代码搜索确认该功能不再调用原生 confirm，专用组件不引用持久化 API。
- 本机应用内浏览器成功打开实际验证副本和完整编辑工作台；自动化右键链路在超长时间轴 DOM 上等待超时，因此没有
...
## 2026-09-01：R2.5f2a 审核范围点击与定位语义修正

### 根因与交互拆分

- 问题来自 Timeline 审核块与右侧审核列表共用 `focusAnnotationReviewScope()`：普通点击审核块也会 `seekTo(startTime)`，
  同时建立横向 focus 请求。旧 focus effect 使用 `getCanvasX(start) - 120px` 固定偏移，而悬浮轨道头宽 164px，因此目标
  容易被推到视口前部并落在轨道头遮挡区，播放头也发生用户没有要求的跳转。
- 本轮删除 `TimelineProps.onSelectReviewRange` 及 App 对应回调。审核、评论、反馈块普通点击现在只
  `preventDefault + stopPropagation`，用于阻止底层时间轴空白点击 seek；右键菜单仍可撤销/撤回，并保留用户显式选择的
  “定位到该范围”。右侧原生历史和关联包记录继续调用唯一 `onNavigate`，不会复制第二套审核状态或导航入口。
- 右侧/显式定位仍把播放头放到范围起点，方便从标注范围开头审阅；横向视口请求增加 `center-range` 语义。短范围以
  实际容器宽减去 164px 轨道头得到内容区，并将范围中心对准内容区中心；范围像素宽超过内容区时，将起点放在轨道头
  右侧 24px。首尾位置统一 clamp，现有句级和启动定位继续使用原 start 对齐，没有被审核专用算法改变。

### 代码、清理与验证

- 新增纯 `getTimelineFocusScrollLeft()`，Timeline 只提供 DOM 几何事实并执行原有 260ms 动画；算法处理短/长范围、
  小视口、坏数值和最大 scrollLeft。没有改变播放控制器、拖拽/吸附/缩放、审核记录、ProjectData 或服务器 API；旧固定
  审核偏移和无调用者 callback 已清理，复杂几何旁补充中文功能注释，未新增依赖。
- 定位算法测试 4/4，审核视图/分页/导出回归 16/16，完整 `npm run build` 与 `git diff --check` 通过；只有既有 Vite
  主 chunk 大小提示。首次构建发现 optional padding 的严格类型声明缺口，已把数值归一化 helper 明确为接受
  `number | undefined` 并重新跑完全部验证。
- 应用内浏览器打开实际验证副本：点击 Timeline 审核块前后 `scrollLeft` 均为 0、播放头样式均为 `left: 412px`；点击
  右侧 1:45.305–1:52.352 记录后，播放头到范围起点，目标审核块落在容器 `464–605px`，中心约 534px，正好位于
  903px 视口扣除 164px 轨道头后的内容中心，控制台无 error/warning。**已完成**：代码、测试、构建、浏览器与文档；
  **待用户体验**：确认长范围左对齐余量和 260ms 滚动速度符合实际审阅习惯。本轮未部署生产服务器。

## 2026-09-01：R2.5f2b 循环范围右键播放与审核快捷入口

### 交互与业务边界

- Timeline 循环范围块新增独立右键事件：只阻止浏览器菜单和底层时间轴事件，并把当前显示范围、屏幕坐标交给 App；没有
  改动左键切换循环、整体拖动、两端缩放、空白拖出范围或双击清除。App 的 `TimelineContextMenu` 增加明确
  `loop-range` 联合类型，菜单复用现有循环开关、`playLoopFromRangeStart()`、`playLoopRangeOnce()` 与清除函数，没有复制
  媒体播放状态机。
- 菜单按有效权限显示审核动作：`review` 对应标注确认与审核评论，`write` 对应编辑反馈；权限映射提取为
  `getAvailableAnnotationReviewCreateModes()`，右键菜单和右侧审核面板共用。类型检查同时暴露旧的“排除菜单种类后假定必有
  id/trackId”写法，本轮将新建类型入口收紧为 `action | custom-block | attached-point` 正向联合，避免以后新增非实体菜单再次
  混入块业务。
- 根据用户追加要求，审核快捷入口不展开或跳转右侧面板，而是在原位打开 `AnnotationReviewCreateDialog`。确认备注可空，
  评论/反馈正文必填；窗口显示右键时的范围快照并使用 `targets: all`。提交继续调用唯一 `useAnnotationReviews` mutation，
  共用未保存改动、加载、服务器 revision 和权限阻断；API 错误留在窗口中，成功才关闭并刷新历史。高级领域/轨道目标仍由
  原右侧面板承载，没有复制完整高级表单，也没有新增接口、文件格式或审核事实类型。

### 清理、自审与验证

- 早期实现曾短暂尝试向右侧面板发送创建模式请求；用户明确改为弹窗后，相关 request state、ref、effect、聚焦 prop 和
  App 打开面板函数已全部删除。最终只保留共享权限 helper、独立弹窗与一条提交接线，代码搜索无残留，不存在双 mutation
  管道。复杂事件阻断、范围快照和权限共享处补充中文注释，未新增依赖。
- 审核视图/分页/导出回归 17/17，新增权限组合覆盖无权限、仅审核、仅编辑和审核加编辑；完整 `npm run build` 与
  `git diff --check` 通过，只有既有 Vite 主 chunk 大小提示。
- 应用内浏览器在验证副本拖出 6.050–16.050 秒循环范围。右键前后循环块 class 均为 active，菜单显示循环开关、两种范围
...
## 2026-09-03：v0902 平台优先三方合并工具（待人工验收）

- 新增超管 CLI 的 dry-run、断点执行、事后验证和隔离 live-test 流程。正式文件使用 `bulk_import`
  变更租约和 `annotation.project.snapshot.boundary`，保留原资源 ID、VOD 绑定和恢复历史；旧计划、revision
  漂移、哈希不同或引用不完整均 fail closed。
- 合并核心统一为“平台字段与 revision 1 v0901 相同则取 v0902，否则取平台”。时间线实体先按稳定 ID
  对应；离线重建 ID 时才在同父级、数量相同、时间位置唯一的前提下按时序对应。数量不同、时间重合或 ID
  重建同时换父级时阻断，不以全局数组下标猜测。区间元素的 `startTime + endTime` 是联动子字段：平台改过任意
  一端时两端均取平台，防止与 v0902 拼接出从未存在或结束早于开始的新区间。
- 真实 099 只读 dry-run 为 1 ready / 0 blocked。平台新建隔离项目
  `__v0902三方合并人工复核_099_v0901_duanqiao_zjq.json_1788435479054-83ddd6ba`，其中保留区间联动后的合并结果与
  原始 v0901、平台当前 v0901、本地 v0902 三份对照。源文件 revision 292 只读，没有保存、改名或覆盖。
- 区间联动后的全量只读 dry-run 发现 90 个已修改 v0901：77 ready、13 blocked。旧算法的 23 个时间倒置阻断全部消失；
  剩余为 8 个扫描中 revision 漂移、1 个无歧义对应失败和 5 个 ID/引用完整性失败，其中 057 同时命中后两类。
- 合并专项测试、shared/document-model build 和 Node TypeScript 检查通过。**待人工验收**：在上述 099
  隔离项目检查开头字级对齐、句级角色和“青儿啊/娘娘”换句结构。人工确认前不提交，也不对任何已有正式文件执行替换。

## 2026-09-04：v0902 工尺修改整折跳过策略

- 对 030、044、057、065a、065b、176 做只读三方诊断：原始 v0901、平台当前和本地 v0902 各自引用完整，606 个
  工尺悬空引用仅在旧合并算法生成的结果中出现。平台新增工尺仍指向 v0901 字 ID，而 v0902 已重建全部字 ID；606 个
  父字均能在同句同时间序位唯一映射且字面一致，确认源文件没有损坏，问题来自跨集合引用 ID 没有随实体身份迁移。
- 按用户选择不自动重映射工尺引用。平台 `gongcheAnnotations` 相对 revision 1 v0901 有任何新增、删除、重排、父块、
  工尺块或符号字段变化时，整折以 `skipped` 留在计划中，不生成合并正文、不保存、不改名，也不阻断其余安全折执行。
  计划格式升至 v2，摘要和终端分别列出 `skippedCount` / `skippedRows`，execute 状态与 verify 均排除跳过行。
- 此次诊断和策略实现没有写入已有平台文件。调查期间 057、065a、065b 的线上 revision 继续增长，正式维护窗口必须重新
  dry-run 并使用新 fingerprint；此前任何统计和计划只能作为诊断记录，不能直接执行。
- 策略实现后的 2026-09-04 只读全量 dry-run 扫到 90 个已修改 v0901：83 ready、5 skipped、2 blocked；跳过项为
  030、057、065a、065b、176，两个阻断项 122、269 均为扫描中的 revision 漂移。此前曾有一个平台工尺块的 044 已推进到
  revision 1229，当前 `gongcheAnnotations` 与 revision 1 相同，因此按统一数据规则恢复为 ready，而不是依赖历史名单跳过。
  计划 v2 的数量等式和全部跳过行均已核对：跳过行没有合并哈希、没有其他 blocker，不会进入状态文件。
- 最终风险审查后把执行状态格式升到 v2：解析时要求状态资源集合与计划的全部可执行行完全一致，并逐行校验 UUID、输入
  revision、目标名、合并哈希及各状态允许的精确字段；`completed` 续跑还会依据 committed revision 重新核对名称、父级、
  媒体绑定和正文哈希。若当前 revision 已被后续编辑推进，则精确读取 committed revision 的恢复快照作为服务器证据；
  不能靠误写或完整伪造本地 `completed` 状态跳过真实提交。
- 回退手册补充 write 权限、`save_and_rename` / `rename_only` / `skipped` 的不同恢复路径、Inspector 最近 50 条窗口、按 plan
  `currentRevision` 精确定位快照、后续用户编辑会退出活动正文但先形成保护快照，以及当前没有批量 rollback 命令。此次加固
  仅修改本地代码、测试和文档，没有读取或写入平台资源。

## 2026-09-04：005 已开始标注文件隔离线上测试

- 选择正式项目中的 `005_v0901_guichi_wjj_b.json`（源资源
  `55f9f503-66db-4771-92e5-7fa8f3b1ffdd`，测试时 revision 254）执行 live-test。CLI 只读取得源文件、revision 1
  快照和本地 v0902，在新建隔离项目
  `__v0902三方合并人工复核_005_v0901_guichi_wjj_b.json_1788455250942-a818fd3f` 中写入合并文件及三份对照；没有保存、
  改名或覆盖正式源文件。
- 隔离项目 ID 为 `98aadaf3-40a0-4a04-8155-bfda5e8e1640`。合并文件
  `005_v0902_guichi_wjj_b.json` 的资源 ID 为 `4d9ac80f-a15c-4e23-ab95-bd9085eeead8`，最终 revision 3；测试确认资源 ID
  和 VOD 绑定在合并保存及改名后保持不变。项目内另有原始 v0901、平台修改版本和本地 v0902 三份只读人工对照文件。
- 合并审计统计为：平台正文 1857 字、原始及 v0902 各 1850 字；平台和 v0902 的字 ID 没有交集，因此按保守规则完整保留
  平台字集合。字段来源包含三方相同 905、v0902 更新 2725、平台修改 8、平台区间联动 3、冲突取平台 51；自动检查确认
  预期字段值和平台结构变化全部进入结果。平台新增/改写的“果然厉害吓”“果有神道”等结构得到保留，未改区域采用 v0902。
  **待人工验收**：在该隔离项目查看合并文件与三份对照；通过前仍不提交代码，也不执行正式文件替换。

## 2026-09-04：021 高 revision 已标注文件隔离线上测试

- 按用户要求换用不同折目，选择 `021_v0901_youyuanjingmeng_zjq_a.json`（正式源资源
  `7be18961-2eb4-41d4-99ad-50f45c503d09`，测试时 revision 806）。最新全量只读计划确认其为 ready，且没有命中平台工尺
  修改整折跳过规则；正式源文件全程只读。
- 新建隔离项目 `__v0902三方合并人工复核_021_v0901_youyuanjingmeng_zjq_a.json_1788456139543-7a28e915`
  （项目 ID `77bb2806-f155-47e7-9fd2-1d39191ba7cf`），内含合并结果及原始 v0901、平台修改 v0901、本地 v0902 三份对照。
  合并结果资源 ID 为 `9a67865e-3c1d-4dee-829f-9dd72fc63762`，最终 revision 3，资源 ID 与 VOD 绑定检查通过。
- 平台已把字数从 670 调整为 668，并改写/拆分多处句子；合并结果保持 668 字，保留“春香”“可曾吩咐花郎”“放下”“好天气也”等
  平台结构。字段审计为三方相同 280、v0902 更新 649、平台修改 73、平台区间联动 1、冲突取平台 259；句级唱念与角色的 68 个
  平台修改字段全部保留。自动核对 `allExpectedValuesApplied` 和 `allStructuralChangesApplied` 均为 true，等待人工复核。

## 2026-09-04：v0902 合并工具交接前最终加固

- 双轴交付审查确认当前代码改动均属于 v0902 三方合并及其精确恢复快照 API，没有把本地 v0902 数据目录或盘点 CSV 纳入
  代码交付。根据审查补齐四项交接边界：正式 plan/state 改用仓库外持久受控目录；锁文件记录 PID、启动时间和 fingerprint，
  手册提供确认旧进程退出后保留式移动 stale lock 的步骤；`/tmp` 只用于 live-test 内部临时文件。
- 工尺跳过判断现在只依赖可信的 revision 1 与平台当前正文，并先于 incoming 规范化/重复 ID 检查。本地 v0902 缺失、损坏、
  重复 ID 或目标名冲突不再把明确应跳过的折目变成整批 blocker；服务器身份、基线缺失或扫描中 revision 漂移仍然 fail closed。
- `verify` 除可执行行外，也逐项检查 skipped 行的原名称、父级、媒体绑定、revision 和正文哈希，要求
  `verifiedSkippedCount = skippedCount`。新增专项测试覆盖工尺优先跳过与跳过行状态核验。
- 交付前 `test:platform-merge-v0902`、Node TypeScript、API build、API 测试 TypeScript 和 `git diff --check` 通过。完整 web build
  仍被工作区未安装但已在 package/lock 声明的 `@radix-ui/react-dropdown-menu` 阻断，并连带产生 Inspector 回调隐式 any；该错误
  在本功能之外且未改依赖文件，交接环境执行 `npm ci` 后需按手册重新跑完整 `npm run build`。数据库集成测试仍需可用测试库。
