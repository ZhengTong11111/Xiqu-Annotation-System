...
- 循环和单次范围播放不再使用按播放倍率计算的固定提前阈值；此前 `1x` 会提前约 40ms 触发，短范围可能只播放到设置范围的一部分。
- 新增独立的 `requestAnimationFrame` 边界监测，直接读取统一播放控制器的实时媒体时钟；只有实际时间到达范围 `end` 后才执行回跳或结束单次播放。
- VOD 异步回跳在当前监测周期内增加单飞保护，避免 seek 尚未完成时重复提交起点跳转；范围、媒体或播放意图变化后，旧监测不能继续调度新的边界检查。
- 修复 P/Tab 初始 `seek + play` 与边界监测的启动竞态：旧播放位置不会再被误判为已到终点，迟到的旧启动请求也不能清除新请求的门禁。
- 继续修复 P 循环回归：每轮回跳都统一通过真实媒体时钟监测 end，并让循环监测以播放器实际暂停状态为准，避免首轮之后停止监测或顺序播放到底。
- 修复首轮后循环只执行一次的竞态：受控循环 seek 不再提前写入 React 的 `currentTime`，避免播放器暂时 paused 时由普通时间同步 effect 追加一个没有 `playAfterSeek` 的后发 seek，覆盖 P 的下一轮播放命令。
- 增加独立的持续循环意图 ref；回跳或缓冲造成短暂 paused 时仍继续轮询，直到 seek 完成并恢复播放。用户主动暂停时会清除该意图，不会强行重新播放。
- 清理会卡死短循环的起点门禁：统一播放控制器的 `seek` Promise 已经是 seek 完成事实，边界监测只读取实际媒体时钟判断 end；不再要求播放头额外停留在 start 附近，避免慢播放器或短区间下永远等不到确认。
- 恢复 P 循环期间切换块的旧有交互：选中开启自动循环范围的下一个块时，先更新临时循环意图，再取消旧边界帧并立即 seek 到新块起点继续播放；暂停状态下只更新范围，不会因为选块意外起播。
- 本轮只改循环起止判断，未修改 VOD 会话鉴权、Web License、外接音轨同步、波形/频谱分析、保存和协作逻辑。

### 验证与边界

- `npm run test:media-playback`：90/90 通过。
- `npm run build:web`：通过；Vite 的大 chunk 提示仍是已有构建警告，不是本轮失败。
- `git diff --check`：通过；本轮未连接、修改或部署服务器。
- 循环范围的保存值仍受时间轴缩放和鼠标像素分辨率影响；VOD 实际音频起播还会受 Aliplayer 分片、解码和浏览器调度影响，代码只消除了前端主动提前截断，不能宣称底层采样绝对零误差。

### 待推进

- 用户应在本机 `localhost` 的 VOD 样例上验证 50--100ms 范围的实际听感和连续回跳；如仍需采样级精度，应单独评估 VOD 短循环缓冲能力，不应再次通过全局同步容差或鉴权链路解决。
