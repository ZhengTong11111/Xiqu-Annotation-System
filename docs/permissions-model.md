...
- `review` 与 `manage_permissions` 必须继续在详细模式中单独设置，不能随“可编辑”自动授予。
- 直接 capability 只有与预设**完全相同**时才能显示为该预设；存在额外、缺失或重复能力时显示为
  “自定义细分权限”，并保留原值，直到用户明确选择预设覆盖或进入详细模式编辑。
...
- 集中窗口继续复用 10.1 的三档 capability 映射，不新增数据库权限等级。`review`、`manage_permissions`、
...
- “不额外授权”只删除当前项目对该账号的直接 ACL，不删除子资源 ACL，也不修改项目自身是否继承父级权限。
